 ...
 * @author taha
 */
public class HavaYoluUlasım extends UlasımYap{
    
    
    public HavaYoluUlasım(int ulasim_id, int satis_id, String sattigimiz_ulke, String satilan_urun, String satilan_renk, String firma_iletisim, String ulasım_yolu, String teslim_edildimi, int ulasım_yolu_uzaklık,int satilan_desi, int ulasım_maliyet, int tahmini_teslimat_gunu) {
    ...
    public HavaYoluUlasım() {
    }
    public HavaYoluUlasım( int satis_id, String sattigimiz_ulke, String satilan_urun, String satilan_renk, String firma_iletisim, String ulasım_yolu, String teslim_edildimi, int ulasım_yolu_uzaklık,int satilan_desi, int ulasım_maliyet, int tahmini_teslimat_gunu) {
    ...
    public int KargoMaliyet(int satis_id,int satılan_desi) {
     int maliyet=0;
        ...
        int ucak_sayısı=KargoyuYukle(paket_sayısı);
        ...
        for(Satis c:veri){
            if(c.getSatis_id()==satis_id && c.getUlasım_yolu().toLowerCase().equals("deniz")){
                maliyet+=c.getUlasım_yolu_uzaklık()*20;
                maliyet*=ucak_sayısı;
            }
        }
        return maliyet;
    ...
    int maliyet=0;
        ...
        List<Satis> veri=s.Verileri_Listeleme();
        for(Satis c:veri){
            if(c.getSatis_id()==satis_id &&c.getUlasım_yolu().toLowerCase().equals("hava")){
                if(c.getSatilan_ulke().toLowerCase().equals("turkiye")){
                    maliyet=goturulen_arac*TurkiyeHavaAlanıUzaklık;
                }
                if(c.getSatilan_ulke().toLowerCase().equals("amerika")){
                    maliyet=goturulen_arac*AmerikaHavaAlanıUzaklık;
                }
            }
        }
        maliyet=maliyet*5;
     ...
     SqlVeriIslemleriSatis s=new SqlVeriIslemleriSatis();
        ...
        for(Satis c:veri){
            if(c.getSatis_id()==satis_id && c.getUlasım_yolu().toLowerCase().equals("hava")){
                ...
                u.VeriEkleme(new HavaYoluUlasım(c.getSatis_id(),c.getSattigimiz_ulke(),c.getSatilan_urun(),c.getSatilan_urun_renk(),
