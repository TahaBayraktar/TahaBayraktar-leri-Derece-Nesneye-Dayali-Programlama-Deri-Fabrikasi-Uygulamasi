...
import AbstractFactory.ISuetDeri;
 ...
 * @author taha
 */
public class TurkiyeSuetFactory extends Uretim implements AbstractSuetFactory{
     private int gun;
    private int SuetBirimMaliyet;

    public TurkiyeSuetFactory() {
        this.gun = gun;
        this.SuetBirimMaliyet = 2;
    }
    @Override
        ...
        for(Depo c:veriDepo){
            if(c.urun_id==id && c.urun_adi.equals("suet") && c.renk.equals(renk) && c.stok_miktari>= istenen_miktar){
                for(Uretim a:veriUretim){
                    if(a.urun_id==id && a.ulke.equals("turkiye")){
                       ...
                       ur.UretimGuncelle("maliyet", "", a.uretim_id, a.maliyet+(istenen_miktar*SuetBirimMaliyet), gun);
                        return new TurkiyeSuet(renk,id,istenen_miktar); 
                ...
                StokGüncelleme("stok_miktari", "", id, c.stok_miktari-istenen_miktar, 0); 
                        ...
                        + "üretim yapılmıştır.");
                        u.VeriEkleme(new TurkiyeSuet(renk,id,istenen_miktar), SuetBirimMaliyet*istenen_miktar);
                        return new TurkiyeSuet(renk,id,istenen_miktar); 
            }
        }
