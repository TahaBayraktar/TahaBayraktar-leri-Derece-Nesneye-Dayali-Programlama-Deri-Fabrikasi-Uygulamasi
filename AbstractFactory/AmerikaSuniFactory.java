 ...
 * @author taha
 */
public class AmerikaSuniFactory extends Uretim implements AbstractSuniFactory{
   private int gun;
    private int SuniBirimMaliyet;
   
    public AmerikaSuniFactory(){
        
        this.SuniBirimMaliyet=2;
    }
    @Override
        ...
        for(Depo c:veriDepo){
            if(c.urun_id==id && c.urun_adi.equals("suni") && c.renk.equals(renk) && c.stok_miktari>= istenen_miktar){
                ...
                for(Uretim a:veriUretim){
                    if(a.urun_id==id && a.ulke.equals("amerika")){
                       ...
                       ur.UretimGuncelle("maliyet", "", a.uretim_id, a.maliyet+(istenen_miktar*SuniBirimMaliyet), gun);
                        return new AmerikaSuni(renk,id,istenen_miktar);   
                    }
                ...
                StokGüncelleme("stok_miktari", "", id, c.stok_miktari-istenen_miktar, 0); 
                        ...
                        + "üretim yapılmıştır.");
                        u.VeriEkleme(new AmerikaSuni(renk,id,istenen_miktar), SuniBirimMaliyet*istenen_miktar);
                        return new AmerikaSuni(renk,id,istenen_miktar); 
            }
        }
